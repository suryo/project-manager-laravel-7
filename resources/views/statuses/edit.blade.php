        ...
        <div class="col-md-8">
            <div class="card card-custom border-0 shadow-sm">
                <div class="card-header bg-white border-0 pt-4 px-4">
                    <h4 class="fw-bold text-primary mb-0">Edit Status</h4>
                </div>
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('statuses.update', $status) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="name" type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" name="name" value="{{ old('name', $status->name) }}" required autofocus>
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                @foreach(['primary', 'secondary', 'success', 'danger', 'warning', 'info', 'light', 'dark'] as $color)
                                    <div>
                                        <input type="radio" class="btn-check" name="color" id="color-{{ $color }}" value="{{ $color }}" {{ old('color', $status->color) == $color ? 'checked' : '' }} required>
