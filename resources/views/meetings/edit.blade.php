                ...
                <div class="card-body">
                    <form action="{{ route('meetings.update', $meeting) }}" method="POST" id="meeting-form">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('department_id') is-invalid @enderror" id="department_id" name="department_id" required>
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->id }}" {{ old('department_id', $meeting->department_id) == $dept->id ? 'selected' : '' }}>
                                        {{ $dept->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('meeting_date') is-invalid @enderror" id="meeting_date" name="meeting_date" value="{{ old('meeting_date', $meeting->meeting_date->format('Y-m-d\TH:i')) }}" required>
                                    @error('meeting_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('location') is-invalid @enderror" id="location" name="location" value="{{ old('location', $meeting->location) }}" placeholder="e.g., Meeting Room A">
                                    @error('location')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div id="editor" style="height: 200px; background: #fff;">{!! old('notes', $meeting->notes) !!}</div>
                            <input type="hidden" name="notes" id="notes" value="{{ old('notes', $meeting->notes) }}">
                            @error('notes')
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('meetings.show', $meeting) }}" class="btn btn-light me-md-2">Cancel</a>
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="bi bi-save me-2"></i>Update Meeting
                            </button>
                        </div>
                    </form>
                </div>
